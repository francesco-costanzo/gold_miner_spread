...
def get_most_recent_trading_day():
    """Return the most recent weekday date in ``YYYY-MM-DD`` format."""
...
def load_data(ticker, start="2006-06-01"):
    """Return price series for ``ticker`` from ``start`` to the latest date."""
...
def lag_matrix(series, lags=10):
    """Return DataFrame of ``lags`` past values of ``series``."""
    lagged_data = []
...
def rolling_corr_change(series1, series2, window=15):
    """Return 1 when the rolling correlation declines, otherwise 0."""
...
def annualized_return(returns, periods_per_year=252):
    """Return the annualized return for ``returns``."""
...
def annualized_std(returns, periods_per_year=252):
    """Return the annualized standard deviation of ``returns``."""
    return np.std(returns) * np.sqrt(periods_per_year)


def sharpe_ratio_func(returns, periods_per_year=252):
    """Return the Sharpe ratio calculated from ``returns``."""
    if np.std(returns) == 0:
        return np.nan
