...
def train_model(train_features, train_targets):
    """Create and fit a PySR symbolic regression model."""
    model = PySRRegressor(
        niterations=100,
        binary_operators=["+", "-", "*", "/"],
...
def backtest_and_plot(model, test_features, test_targets, corr_filter_series):
    """Run backtest, plot results, and print metrics."""
    top_equations = model.equations_.sort_values("loss").head(10)
    pred_list = [model.predict(test_features.values, index=idx) for idx in top_equations.index]
    ...
    metrics_df = pd.DataFrame(
        {
            "Cumulative Return (%)": [cumulative_strategy[-1] * 100, cumulative_benchmark[-1] * 100],
            "Annualized Return (%)": [
            ...
            "Sharpe Ratio": [sharpe_ratio_func(strategy_returns), sharpe_ratio_func(benchmark_returns)],
        },
...
def main():
    ...
    backtest_and_plot(model, test_features, test_targets, corr_filter_series)
